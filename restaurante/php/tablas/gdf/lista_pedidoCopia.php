...
<div class="row">
    <div class="col-12 col-lg-12">
        <?php
            $sql_query = "SELECT * FROM `tb_carrito` as c
             INNER JOIN `tb_carta` as ca on c.fk_id_comida=ca.id_comida WHERE c.fk_id_usuario='$iduser'";
        ...
        ?>
          <button class="btn btn-outline-danger mr-3" onclick="AgregarPedidos(<?php echo $iden ?>,'<?php echo $iduser ?>')">Finalizar pedido</button>
    </div>
</div>
<script>
    function AgregarPedidos(id,iduser) {
        alert(id);
        alert(iduser);
        ...
        var settings = {
        "url": "http://localhost/restaurante-salesiano/restaurante/php/crud/gdf/restaurante/agregarPedido.php",
        "method": "POST",
        "timeout": 0,
        "headers": {
        ...
        if(response==1){
            ...
            location.href ="http://localhost/restaurante-salesiano/restaurante/agregarPedido.php";
        }else{
            console.log('error');
        }
        });
