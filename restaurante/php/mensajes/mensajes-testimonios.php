...
<div class="row">
    ...
    $totalQuery = $conexion->query("select count(*) total_mensajes_testimonios from p_t where p_t = 'testimonio' and eliminado = '1' ") or die($conexion->error);
    $totalMensajes = mysqli_fetch_row($totalQuery);
    $totalBotones = round($totalMensajes[0] / $limite);
    if (isset($_GET['limite'])) {
        $resultado = $conexion->query("select count(*) total_mensajes_peticiones from p_t where p_t = 'testimonio' and eliminado = '1' order by id DESC limit " . $_GET['limite'] . "," . $limite) or die($conexion->error);
    } else {
        $resultado = $conexion->query("select count(*) total_mensajes_peticiones from p_t where p_t = 'testimonio' and eliminado = '1'  order by id DESC limit " . $limite) or die($conexion->error);
    }

    $sql_total = "select count(*) total_mensajes_peticiones from p_t  where p_t = 'testimonio' and eliminado = '1' ";
    $result_set = mysqli_query($conexion, $sql_total);
    $total_cartelera = mysqli_fetch_assoc($result_set);


    $sql_query = "SELECT * FROM p_t where p_t = 'testimonio' and eliminado = '1' ";
...
<nav>
    <ul class="pagination">
        ...
        <?php
            ...
            if ($_GET['limite'] > 0) {
                echo ' <li class="page-item disabled"><a href="mensajes-testimonios.php?limite=' . ($_GET['limite'] - 10) . '" class="page-link">&lt;</a></li>';
            }
        }

        for ($k = 0; $k < $totalBotones; $k++) {
            echo  '<li class="page-item"><a href="mensajes-testimonios.php?limite=' . ($k * 10) . '" class="page-link">' . ($k + 1) . '</a></li>';
        }
        if (isset($_GET['limite'])) {
            if ($_GET['limite'] + 10 < $totalBotones * 10) {
                echo ' <li class="page-item"><a href="mensajes-testimonios.php?limite=' . ($_GET['limite'] + 10) . '" class="page-link"> &gt;</a></li>';
            }
        } else {
            echo ' <li class="page-item"><a href="mensajes-testimonios.php?limite=10" class="page-link">&gt;</a></li>';
        }
        ?>
...
<div class="modal fade" id="modalVer" tabindex="-1" role="dialog" aria-labelledby="modalVer" aria-hidden="true">
    <div style="background:#339EFF;" class="modal-dialog">
